...
def run_realtime():
    ...
    while True:
        ...
        if tracker.is_ready():
            ...
            cv2.putText(frame, f"vx={vxk:.1f} vy={vyk:.1f}",
                        (10, 55), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 2)
            cv2.putText(frame, f"ax={axk:.1f} ay={ayk:.1f}",
                        (10, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 2)

        # ── Проверяем прогнозы, срок которых наступил ─────────
        if current_pos is not None:
            while pending_predictions and \
                  current_time >= pending_predictions[0]["target_time"]:
        ...
        cv2.putText(
            frame,
            f"Points: {len(tracker)}/{HISTORY_SIZE} | Mode: {mode} | "
            f"Det: {det_rate:.1f}%",
            (10, 25),
        ...
        if checked >= MIN_CHECKS_TO_SHOW:
            ...
            cv2.putText(frame, f"Checked: {checked}",
                        (10, h - 85),
            ...
            cv2.putText(frame,
                        ...
                        cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 255), 2)
            cv2.putText(frame,
                        f"dX: {mean_dx:.1f}px | dY: {mean_dy:.1f}px | "
