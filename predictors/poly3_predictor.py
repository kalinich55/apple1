...
from config import POLY3_HISTORY_SIZE, POLY3_MIN_POINTS
from .base_predictor import BasePredictor


class Poly3Predictor(BasePredictor):
    def __init__(self):
        self.history = deque(maxlen=POLY3_HISTORY_SIZE)
    ...
    def is_ready(self):
        return len(self.history) >= POLY3_MIN_POINTS

    def _fit(self):
        if len(self.history) < POLY3_MIN_POINTS:
            return None
        ...
        if (
            ...
            or len(np.unique(ts)) < POLY3_MIN_POINTS
            or np.ptp(ts) <= 1e-6
        ):
            return None
        ...
        try:
            with warnings.catch_warnings():
                ...
                coef_x = np.polyfit(ts, xs, 3)
                coef_y = np.polyfit(ts, ys, 3)
